...
int recommend(RecNode *root){
	int max=0,ymax=0;	
	int i,j,r,moveL=0,moveR=0,PRotate,temp=0;
	int h,w;
	int x,y;
	int res=0;
	int n=root->lv;	
		...
		for(x=moveL;x<=moveR;++i,++x){
			...
			for(h=0;h<HEIGHT;++h)
				for(w=0;w<WIDTH;++w)
					t->f[h][w]=root->f[h][w];
			y=0;
			if(CheckToMove(t->f,nextBlock[n],r,y,x)){	
...
int modified_recommend(RecNode *root){
	...
	int i,j,r,PMove=0,moveL=0,moveR=0,PRotate,temp=0;
	int h,w;
	int x,y;
	int res=0;
	int n=root->lv;	
	...
	for(r=0;r<PRotate;++r){
		...
		/*블럭이 가장자리에 더 많이 갈 수 있도록 점수값을 보정*/
